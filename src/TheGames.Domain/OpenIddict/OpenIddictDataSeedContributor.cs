...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("TheGames") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "TheGames", DisplayName = "The Games API", Resources = { "TheGames" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["TheGames_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["TheGames_Web:RootUrl"]!.EnsureEndsWith('/');

            /* TheGames_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["TheGames_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["TheGames_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["TheGames_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        // App Client
        var appClientId = configurationSection["TheGames_App:ClientId"];
        if (!appClientId.IsNullOrWhiteSpace())
        {
                await CreateApplicationAsync(
                    ...
                    secret: configurationSection["TheGames_App:ClientSecret"] ?? "1q2w3e*",
